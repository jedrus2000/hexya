...
import (
	...
	"github.com/jedrus2000/hexya/hexya/src/tools/generate"
...
func replacePoolDirInGoMod(poolDir string) {
	runCommand("go", "mod", "edit", "-replace", fmt.Sprintf("github.com/jedrus2000/hexya/pool@v0.2.0=%s", poolDir))
}
...
func copyGoModReplaces(poolDir string) {
	...
	for _, repl := range replaces.Replace {
		if repl.Old.Path == "github.com/jedrus2000/hexya/pool" {
			continue
		}
...
module github.com/jedrus2000/hexya/pool
`))
...
import (
	...
	"github.com/jedrus2000/hexya/hexya/cmd"
	"github.com/spf13/cobra"
{{ range .Imports }}	_ "{{ . }}"
{{ end }}
)
